...
if (typeof NProgress != 'undefined') {
	...
	function init_chart_doughnut(arr1,label1,chartId,color){
	   ...
	   /* CUSTOM NOTIFICATION */
		...
		function init_echarts() {
			...
			if ($('#mainb').length ){
				  ...
				  echartBar.setOption({
					title: {
					  text: 'Similarity Comparison Graph',
					...
					series: [{
					  ...
					  markPoint: {
						data: [{
						  type: 'max',
						  name: 'Max Dissimilarity'
						}, {
						  type: 'min',
						  name: 'Min Dissimilarity'
						}]
					  },
					  markLine: {
						data: [{
						  type: 'average',
						  name: 'Avg Dissimilarity'
						}]
					  }
					}]
				  });
...
function init_DoughnutChartData(pieData,labels,vc,color){
	for(var i=0;i<vc;i++){
		init_chart_doughnut(pieData[i],labels[i],'doughChart'+i,color);
	}
	init_sparklines();
		init_flot_chart();
		init_sidebar();
...
function init_BarChartData(dbVideoNames,dbVidSimilarity,dbVidDisSimilarity){
	dbVideoIDs = dbVideoNames;
	console.log(dbVidSimilarity);
	dbSim = dbVidSimilarity;
	dbDisSim=dbVidDisSimilarity;
	init_echarts();
	init_flot_chart();
	init_charts();
		init_sidebar();
